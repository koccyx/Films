...
export default function Films() {
  ...
  useEffect(() => {
    fetchFilms(filters.state.sortOption.value, filters.state.page).then((data) => {
      filmsContext.handleFilms(data.films);
      filtersContext.handleTotalPages(data.pages);
      console.log(data.films);
    });
  }, [filtersContext.state.genreList, filtersContext.state.selectedYears, filtersContext.state.sortOption, filtersContext.state.page]);
